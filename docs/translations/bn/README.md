...
**বর্তমান অগ্রগতি**: M1–M13 সব সম্পন্ন —— M1 প্রশাসন প্যানেল (ক্যারিয়ার / ক্রেডেনশিয়াল / কোয়েরি রেকর্ড / সাবস্ক্রিপশন CRUD), M2 কোয়েরি গেটওয়ে (সম্পূর্ণ বাহ্যিক API চেইন), M3 কলব্যাক সাবস্ক্রিপশন, M4 মনিটরিং ও পরিসংখ্যান, M5 বাহ্যিক OpenAPI ডকুমেন্টেশন, M6 পাঁচটি ক্লায়েন্ট SDK, M7 ক্লায়েন্ট পোর্টাল (রেজিস্ট্রেশন / অ্যাপ / প্ল্যান / অর্ডার), M8 পেমেন্ট (Stripe / PayPal), M9 ক্রিপ্টো (USDT TRC20 / BEP20 / ERC20), M10 পেমেন্ট মেথড কনফিগারেশন, M11 গেটওয়ে সিকিউরিটি মিডলওয়্যার, M12 CDN পরিকল্পনা (Cloudflare + ক্যাশ হেডার), M13 CDN প্রদানকারী ব্যবস্থাপনা. ক্লায়েন্ট → e-cat → worker → ক্যারিয়ার ট্র্যাকিং কোয়েরি চেইন প্রদর্শনযোগ্য, এবং পাঁচটি নির্ভরতা-শূন্য SDK কপি-এন্ড-ইউজ প্রস্তুত।

**প্রকল্পের মাস্কট E-Cat** —— ট্র্যাকিং রাডার টুপি পরে একটি পার্সেল ধরে থাকা বিড়াল। একটি স্বয়ংসম্পূর্ণ SVG ভেক্টর ফাইল, যাতে শ্বাস / চোখের পলক / লেজ দোলানো / রাডার স্ক্যান অ্যানিমেশন অন্তর্ভুক্ত এবং `prefers-reduced-motion` সমর্থিত। `docs/diagrams/mascot.svg` চরিত্রটির একমাত্র উৎস ফাইল; এটি অ্যাডমিন ইনস্টল উইজার্ড (`/install`), 404 / 5xx ত্রুটি পৃষ্ঠা, Flutter অ্যাডমিন (লগইন পৃষ্ঠা + ওয়েব প্রথম-রেন্ডার লোডার), HarmonyOS ক্লায়েন্ট লগইন পৃষ্ঠা, API ডকুমেন্টেশনের কোণার ব্যাজ এবং সাইট favicon-এ যুক্ত করা হয়েছে।
...
- **ক্লায়েন্ট পোর্টাল ও বিলিং**（M7–M10）：ক্লায়েন্ট রেজিস্ট্রেশন / লগইন (client JWT admin থেকে আলাদা), X-API-Key নিজে সেটসহ অ্যাপ ব্যবস্থাপনা, প্ল্যান / অর্ডার API; Stripe / PayPal + USDT TRC20 / BEP20 / ERC20 ক্রিপ্টো পেমেন্ট, Stripe পেমেন্ট মেথড (Apple Pay / Google Pay / Klarna / SEPA ইত্যাদি) কনফিগারেশনেই কার্যকর;
- **CDN এক্সিলারেশন**（M12/M13）：Cloudflare ফ্রি প্ল্যান সম্পূর্ণ HTTPS + স্ট্যাটিক অ্যাসেটের এজ ক্যাশ, CDN প্রদানকারী / ডোমেইন / কী অ্যাডমিন প্যানেলে কনফিগারযোগ্য (কী এনক্রিপ্টেড);
...
ব্যবস্থাপনা পৃষ্ঠা (ব্রাউজার) → `/admin/*`：JWT + RBAC পারমিশন + অপারেশন অডিট, কভার করে carrier / carrier-credential / tracking-query / callback-subscription / statistics / client / client-app / plan / order / cdn-provider।
...
- **পেমেন্ট নিরাপত্তা** (M8/M10)：Stripe / PayPal ওয়েবহুক যাচাই (HMAC-SHA256 / verify-webhook-signature)、স্বয়ংক্রিয় অর্ডার নিশ্চিতকরণ + admin ম্যানুয়াল ফলব্যাক；পেমেন্ট কী `Encryptable` দিয়ে এনক্রিপ্ট করে `logistics_system_config`-এ সংরক্ষণ；
- **ক্রিপ্টো পেমেন্ট যাচাই** (M9)：USDT TRC20 Tronscan API দিয়ে স্বয়ংক্রিয় যাচাই；BEP20 / ERC20 ম্যানুয়াল নিশ্চিতকরণ；
- **ক্লায়েন্ট কী নিরাপত্তা** (M7)：X-API-Key ক্লায়েন্ট নিজেই সেট করে (≥১৬ অক্ষর)、sha256 হিসেবে সংরক্ষণ —— প্লেইনটেক্সট শুধু তৈরি করার সময় একবার ফেরত দেওয়া হয়；ক্লায়েন্ট JWT (token_type=client) admin JWT থেকে আলাদা；
- **গেটওয়ে আক্রমণ সনাক্তকরণ**（M11）：`ecat-security` SecurityBodyLayer গেটওয়েতে সংহত (ইনজেকশন / প্রোটোকল / ডেটা সিরিয়ালাইজেশন / ফাইল / সংবেদনশীল ডেটা ফাঁস ডিটেক্টর); আক্রমণের পেলোড গেটওয়ে স্তরেই আটকানো হয়, অ্যাপ্লিকেশন স্তরের নিরাপত্তা প্যাকেজ ব্যাকস্টপ হিসেবে;
- **CDN নিরাপত্তা**（M12）：Cloudflare ফ্রি প্ল্যান সম্পূর্ণ HTTPS + দ্বি-স্তর WAF (এজ ম্যানেজড রুল + গেটওয়ে অ্যাপ্লিকেশন স্তর সনাক্তকরণ); Tunnel অরিজিনে সোর্স শূন্য এক্সপোজার; কলব্যাক কেবল DNS সাবডোমেইন সরাসরি সংযোগে, CDN ব্যর্থতায় অর্ডার হারানো এড়ানো; রেট লিমিট X-API-Key অনুযায়ী, CDN এজ আইপি দ্বারা প্রভাবিত নয়; প্রমাণিত এন্ডপয়েন্ট সর্বদা no-store, ক্রস-ইউজার ক্যাশ মিক্সিং প্রতিরোধ;
- **CDN ক্রেডেনশিয়াল ব্যবস্থাপনা**（M13）：CDN প্রদানকারীর access_key / access_secret `Encryptable` দিয়ে এনক্রিপ্ট করে `logistics_cdn_provider` টেবিলে, `/admin/cdn/provider`-এ কনফিগারেশন;

## বৈশিষ্ট্য

<img src="diagrams/description.svg" alt="প্ল্যাটফর্ম বৈশিষ্ট্য" width="100%">

- **একীভূত ট্র্যাকিং কোয়েরি: একটি ট্র্যাকিং নম্বর দিয়ে সারাবিশ্বে — 187টি নম্বর-প্যাটার্ন নিয়ম স্বয়ংক্রিয়ভাবে অভ্যন্তরীণ / আন্তর্জাতিক চ্যানেল ও ক্যারিয়ার শনাক্ত করে, 209টি ক্যারিয়ার অ্যাডাপ্টার `TrackStatus`-এর 7টি মানক অবস্থায় আউটপুট একীভূত করে;**
- **বহু-ক্যারিয়ার সংযোগ: 45টি অভ্যন্তরীণ + 164টি আন্তর্জাতিক অ্যাডাপ্টার, DHL / FedEx / UPS / USPS ও জাতীয় ডাক S10-এর পূর্ণ কভারেজ, ক্রেডেনশিয়াল এনক্রিপ্টেড, কোনো হার্ডকোডেড কী নেই;**
- **অ্যাডমিন RBAC: JWT + ব্ল্যাকলিস্ট + method.path-স্তরের পারমিশন + পূর্ণ অডিট ট্রেইল, সিকিউরিটি ফিল্টার XSS / SQL ইনজেকশন / CSRF / কমান্ড ইনজেকশন ব্লক করে;**
- **পেমেন্ট ক্লোজড লুপ: Stripe / PayPal + USDT TRC20 / BEP20 / ERC20, webhook সিগনেচার ভেরিফিকেশনে অর্ডার স্বয়ংক্রিয় নিশ্চিত হয়, পেমেন্ট পদ্ধতি কনফিগারেই কার্যকর;**
- **ক্লায়েন্ট পোর্টাল ও প্ল্যান: রেজিস্টার / লগইন / অ্যাপ ম্যানেজমেন্ট / প্ল্যান / অর্ডার API, X-API-Key ক্লায়েন্ট নিজেই সেট করে, ক্লায়েন্ট JWT অ্যাডমিন থেকে সম্পূর্ণ আলাদা;**
- **API গেটওয়ে সুরক্ষা: API-Key অথেনটিকেশন, Redis রেট লিমিট (429), প্রতি-ক্যারিয়ার সার্কিট ব্রেকার (503), SSRF সুরক্ষা, আক্রমণ পেলোড গেটওয়ে স্তরেই ব্লক হয়;**
- **CDN নিরাপদ ডেলিভারি: Cloudflare ফ্রি প্ল্যানে ফুল-সাইট HTTPS + ডাবল WAF + এজ ক্যাশ, Tunnel অরিজিনে কোনো পাবলিক এক্সপোজার নেই;**
- **বহু-ভাষা SDK: Python / PHP / Node.js / Go / Rust-এর জন্য পাঁচটি জিরো-ডিপেন্ডেন্সি SDK, কপি করে চালান।**

## ওয়ান-ক্লিক ইনস্টল

প্রস্তাবিত: Docker Compose দিয়ে এক কমান্ডে ডিপ্লয় — 5টি সার্ভিস (Nginx / PHP / MySQL / Redis / Elasticsearch) হেলথ চেক ও ডেটা পার্সিস্টেন্সসহ চালু হয়:

```bash
bash install.sh
```

রিপোজিটরি ক্লোন করার পর:

```bash
cd integrated-global-logistics   # প্রকল্পের রুটে প্রবেশ
bash install.sh                  # ডিফল্ট পোর্ট 80, NGINX_PORT=8080 দিয়ে বদলানো যায়
```

স্ক্রিপ্ট Docker পরিবেশ যাচাই করে, সব সার্ভিস চালায় এবং হেলথ চেক পোল করে (সর্বোচ্চ 120 সেকেন্ড); প্রস্তুত হলে `http://localhost/install` খুলে ইনস্টল উইজার্ড সম্পূর্ণ করুন (ডেটাবেস ইনিশিয়ালাইজেশন + অ্যাডমিন তৈরি)। বিস্তারিত Docker Compose ডিপ্লয়ের জন্য দেখুন [admin/README.md](../../admin/README.md)।
