...
**حيوان المشروع الأليف E-Cat** —— قطة ترتدي قبعة رادار التتبع وتحمل طردًا. ملف SVG متجهي واحد ومكتفٍ بذاته، مع حركات التنفس / الرمش / تحريك الذيل / مسح الرادار، ويدعم `prefers-reduced-motion`. الملف `docs/diagrams/mascot.svg` هو المصدر الوحيد للشخصية؛ وقد دُمج في معالج تثبيت لوحة الإدارة (`/install`)، وصفحات الأخطاء 404 / 5xx، ولوحة Flutter (صفحة تسجيل الدخول + محمّل أول عرض للويب)، وصفحة تسجيل الدخول لعميل HarmonyOS، وشارة زاوية وثائق API، وأيقونة الموقع favicon.
