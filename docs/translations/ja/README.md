...
**プロジェクトペット E-Cat** —— 追跡レーダーキャップをかぶり荷物を抱えた猫。単一ファイルの SVG ベクターで、呼吸／まばたき／しっぽ振り／レーダースキャンのアニメーションを内蔵し、`prefers-reduced-motion` にも対応。`docs/diagrams/mascot.svg` が唯一の形象ソースファイルで、管理画面のインストールウィザード（`/install`）、404 / 5xx エラーページ、Flutter 管理画面（ログインページ + Web 初回描画ローダー）、HarmonyOS クライアントのログインページ、API ドキュメントのコーナーバッジ、サイト favicon に組み込まれています。
