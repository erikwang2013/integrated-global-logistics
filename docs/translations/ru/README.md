...
**Талисман проекта E-Cat** —— кот в кепке с радаром слежения, держащий посылку. Единый самодостаточный векторный SVG с анимациями дыхания / моргания / виляния хвостом / сканирования радара и поддержкой `prefers-reduced-motion`. `docs/diagrams/mascot.svg` — единственный исходный файл персонажа; он встроен в мастер установки админ-панели (`/install`), страницы ошибок 404 / 5xx, Flutter-панель (страница входа + загрузчик первого рендера веб-версии), страницу входа клиента HarmonyOS, угловой значок документации API и favicon сайта.
