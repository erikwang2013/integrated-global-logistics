...
시스템은 7계층 심층 방어 모델을 채택하여 외부에서 내부로 악성 요청을 계층별로 걸러내며, 어느 한 계층이 무너져도 이후 방어선이 보완하도록 보장합니다.
...
| 계층 | 미들웨어/메커니즘 | 방어 목표 |
|----|--------|---------|
| 1 | SecurityMiddleware (erikwang2013/security-php) | 31종 공격 탐지 + HTTP 메서드 검증 + 요청 본문 크기 제한 + Content-Type 검증 + CSRF + IP 공격 단계 상향 블랙리스트 |
| 2 | Cors | 크로스 도메인 보안 + 응답 보안 헤더 주입 |
| 3 | RateLimit | Redis 슬라이딩 윈도우 레이트 리밋, 무차별 대입 방지 |
...
프론트엔드 3개 계층 (Flutter)에는 별도의 입력 검증이 있지만, 백엔드는 이를 신뢰하지 않으며 각 계층이 독립적으로 방어합니다.

---

## 2. 공격 탐지 엔진

## 2. 공격 탐지 엔진 (erikwang2013/security-php)

공격 탐지는 자체 개발 SecurityMiddleware (erikwang2013/security-php)에서 `erikwang2013/security-php` v1.1+ 전용 보안 패키지로 마이그레이션되었으며, **31종 탐지기**를 제공하여 5대 공격 카테고리를 포괄합니다.

### 2.1 탐지기 분류

**주입 공격 (11종):** XSS, SQL 주입, 명령 주입, NoSQL 주입, LDAP 주입, XPath 주입, JNDI/Log4Shell, SSI 서버 측 포함, GraphQL 주입, SSTI 템플릿 주입

**프로토콜 및 요청 공격 (9종):** SSRF, XXE, HTTP 응답 헤더 주입, Host 헤더 공격, Request Smuggling, Open Redirect, CORS 우회, WebSocket 하이재킹, DNS Rebinding

**HTTP 프로토콜 계층 검증 (6종):** HTTP 메서드 검증(405), 요청 본문 크기 제한(413), Content-Type 검증(415), CSRF Origin 검사, IP 공격 단계 상향 블랙리스트, 민감 데이터 유출 탐지

**데이터 및 직렬화 공격 (5종):** PHP 역직렬화, CSV 수식 주입, 메일 헤더 주입, JWT 공격 (구조 분석), JS Prototype Pollution

**파일 및 경로 공격 (2종):** 경로 탐색, 악성 파일 업로드

### 2.2 처리 모드

각 탐지기는 두 가지 모드를 독립적으로 지원합니다:
- `block` — 공격 탐지 즉시 차단, 설정된 상태 코드 반환
- `log` — 차단하지 않고 로그만 기록 (`header_injection`, `ssti`, `nosql_injection`은 오탐 방지를 위해 기본 log 모드)

### 2.3 IP 공격 단계 상향 블랙리스트

동일 IP가 60초 내에 5회 공격 탐지를 트리거 → 자동으로 15분 차단. 저장 백엔드는 Redis (분산), File (단일 머신 JSON), Cache (고동시성 독립 파일) 중 선택 가능하며, 현재는 Redis 저장으로 구성되어 있습니다.

### 2.4 보안 로그

파일 위치: `runtime/logs/security.log` (자동 로테이션, 10MB/파일)
...
OPTIONS 프리플라이트 요청은 후속 미들웨어 체인에 진입하지 않고 204 빈 응답을 직접 반환합니다.
...
- `frame-ancestors 'none'`: 어떤 페이지의 iframe 삽입도 금지, X-Frame-Options: DENY와 이중 안전장치
- `base-uri 'self'`: `<base>` 태그가 동일 출처만 가리키도록 제한
- `form-action 'self'`: 폼이 동일 출처에만 제출되도록 제한
...
Redis Sorted Set 슬라이딩 윈도우 + Lua 원자적 스크립트, 핵심 연산:
...
Lua 스크립트는 Redis 서버에서 단일 스레드로 실행되어 **본질적으로 원자적**이며, TOCTOU (Time-of-check to Time-of-use) 경쟁 조건을 제거합니다.
...
레이트 리밋 트리거 시 HTTP 429 및 JSON body를 반환합니다:
```json
{"code": 429, "message": "请求过于频繁，请稍后再试", "data": []}
```

모든 응답 (정상 응답 포함)에는 다음 헤더가 포함됩니다:

| 헤더 | 설명 |
|----|------|
| X-RateLimit-Limit | 현재 창에서 허용되는 최대 요청 수 |
| X-RateLimit-Remaining | 현재 창에서 남은 사용 가능 요청 수 |
| X-RateLimit-Reset | 창 재설정 Unix 타임스탬프 |
| Retry-After | 레이트 리밋 시에만 포함, 권장 대기 초수 |
...
짧은 시간 레이트 리밋 보호를 잃더라도 정상 비즈니스 요청을 차단하지 않는 것이 낫습니다.

### 5.4 계정 잠금 메커니즘

로그인 인터페이스는 속도 제한에 더해 특정 사용자를 겨냥한 무차별 대입을 방지하는 **계정 잠금** 메커니즘을 추가로 갖추고 있습니다.
...
잠금 기간 중 모든 로그인 요청은 비밀번호 검증 없이 바로 429를 반환하여 무차별 대입 시도를 완전히 차단합니다.
...
주의: 계정 잠금은 IP가 아닌 `userId` 기준이므로 공격자가 IP를 바꿔도 잠금을 우회할 수 없습니다. IP 레이트 리밋 (10회/분)과 중첩되어 이중 방어를 구성합니다:
- IP 계층: 10회/분 레이트 리밋으로 분산 무차별 대입 차단
...
| 키 | `JWT_SECRET` | 환경 변수 주입, 프로덕션 환경에서 교체 필요 |
...
**Token 추출**: `Authorization: Bearer <token>` 헤더에서 추출, `Bearer ` 접두사를 제거하여 원본 JWT를 얻습니다.
...
**블랙리스트 메커니즘**: 사용자 로그아웃 시 `md5(token)`을 Redis에 기록하며, TTL은 JWT 남은 유효기간으로 설정합니다. Redis 장애 시 블랙리스트 확인은 건너뜁니다 (fail-open). 이때 로그아웃된 Token도 단기간 사용될 수 있지만, JWT 자체의 짧은 유효기간 (2h)이 마지막 안전장치로 작동합니다.

### 6.2 동시 세션 제한

Token 유출 후 여러 기기에서 악용되는 것을 방지하기 위해 시스템은 동일 사용자가 동시에 보유할 수 있는 유효 Token 수를 제한합니다.
...
**강제 로그아웃 시나리오**: 사용자가 4번째 기기에서 로그인하면 1번째 기기의 Token이 강제로 블랙리스트에 추가되어, 이후 요청은 401 "Token已失效，请重新登录"을 반환합니다.

로그아웃 시 현재 Token이 집합에서 제거됩니다. Token이 자연 만료되면 Redis key가 자동으로 소멸하여 집합 구성원도 함께 줄어듭니다.
...
**인가 흐름**:
1. `$request->adminId`가 비어 있음 → 통과 (라우트가 인증 전제를 구성하지 않음)
2. 사용자 → 역할 ( `status=0` 비활성 역할 제외) → 권한 목록 조회
3. 슈퍼 관리자 (`slug = '*'`) → 즉시 통과
4. `strtolower(method) . '.' . trim(path, '/')` 구성 → 권한 목록과 비교
5. 매칭 실패 → 403 `{"code": 403, "message": "无权限访问"}`

**이중 확인**: BaseController가 `confirmPassword()` 메서드를 제공하며, 민감 작업 (사용자 삭제, 데이터 내보내기 등)은 Controller 계층에서 현재 비밀번호 입력을 추가로 요구하여 세션 하이재킹 후의 무단 작업을 방지합니다.
...
OperationLog 미들웨어가 POST / PUT / DELETE 요청에 대해 작업 로그를 자동으로 기록합니다. GET 요청은 기록하지 않습니다.
...
**민감 필드 필터링**: 요청 본문을 재귀 순회하며 다음 필드의 값을 `***`로 대체합니다:

`password`, `old_password`, `new_password`, `new_password_confirmation`, `token`, `secret`, `access_token`, `refresh_token`

**출처 단말 감지** (`detectSource()`): 우선순위 순:

1. `X-Client-Platform` 커스텀 헤더 우선 읽기 (네이티브 클라이언트가 명시적 선언)
2. User-Agent 문자열 추론으로 다운그레이드 (`detectSource()` 메서드 감지 순서):
...
**내결함성**: 로그 쓰기 이상이 비즈니스 요청을 차단하지 않습니다 (`catch (\Throwable)`로 조용히 무시).
...
시스템은 데이터 흐름의 세 단계에 대응하는 3계층 데이터 보호 정책을 채택합니다.

### 8.1 전송 계층 — EncryptionService

`EncryptionService`는 `erikwang2013/encryption` 패키지를 사용하여 API 요청/응답의 민감 필드를 암·복호화합니다.

**기술 세부 사항**:
- 알고리즘: `aes-256-cbc-hmac` (HMAC 서명 포함으로 변조 방지)
...
`AdminUser` 모델은 `Erikwang2013\Encryptable\Encryptable` Eloquent cast를 사용하며, 해당 필드는:

- `email` → Encryptable로 cast, 자동 암·복호화
- `phone` → Encryptable로 cast, 자동 암·복호화
- `id_card` → Encryptable로 cast, 자동 암·복호화

DB 쓰기 시 자동으로 암호화되어 암호문으로 저장되고, 읽기 시 자동 복호화되어 평문으로 반환됩니다. DB 저장 열 타입은 `VARCHAR(500)`이며, 암호문은 base64 형태로 저장됩니다.

**키 체계**: 전송 계층 암호화 (`ENCRYPTION_KEY`)와 독립적으로 `ENCRYPTABLE_KEY`를 사용하며, 하나의 키가 유출되어도 다른 계층이 무너지지 않습니다.

키 순환: `ENCRYPTION_PREVIOUS_KEYS` 환경 변수가 이전 키 목록 (쉼표 구분)을 지원하며, 기존 데이터를 읽을 때 이전 키로 복호화를 시도하고, 다시 쓸 때는 현재 키로 재암호화합니다.

### 8.3 표시 계층 — ID 난독화와 마스킹

**Hashids ID 난독화**: `HashidsService`가 `erikwang2013/hashids` 패키지를 사용합니다.

- 외부 API가 반환하는 DB BIGINT ID를 hash 문자열로 인코딩 (예: `xK3mN9qR2pL7wV8b`)
- 클라이언트는 요청 시 hash 문자열을 전달하고, 백엔드는 자동으로 원본 ID로 디코딩
- 솔트 값 `HASHIDS_SALT` 환경 변수 주입, 솔트가 다르면 인코딩/디코딩 결과가 완전히 달라짐
- hash 최소 길이 16자리, 62자리 영숫자 문자 집합 사용
...
모든 키는 `.env` 환경 변수로 주입되며, 설정 파일은 `getenv()`로 읽고 내장 폴백 기본값 (개발 환경에서만 안전)을 제공합니다.

| 환경 변수 | 용도 | 패키지 | 프로덕션 요구 사항 |
|----------|------|-----|---------|
| JWT_SECRET | JWT 서명 키 | erikwang2013/jwt-webman | 64+ 자리 랜덤 문자열 |
| JWT_ALGORITHM | JWT 서명 알고리즘 | 상동 | HS256 유지 |
| HASHIDS_SALT | ID 인코딩 솔트 값 | erikwang2013/hashids | 랜덤 문자열 |
| SNOWFLAKE_DATACENTER_ID | 데이터센터 ID (0-31) | erikwang2013/snowflake-php | 단일 데이터센터는 기본 유지 |
| ENCRYPTION_KEY | API 전송 계층 암호화 키 | erikwang2013/encryption | 32바이트 랜덤 문자열 |
| ENCRYPTABLE_KEY | DB 저장 계층 암호화 키 | erikwang2013/encryptable | 32바이트 랜덤 문자열, 전송 키와 다름 |

**보안 요구 사항**:
- `.env` 파일은 `.gitignore`에 추가되어 있으며, 버전 저장소에 커밋 절대 금지
- `.env.example`은 공개 템플릿 파일로 실제 키가 포함되지 않음
- 프로덕션 환경에서는 **반드시** 모든 기본 키를 랜덤 문자열로 교체
...
시스템은 `/.well-known/security.txt`에서 RFC 9116 표준에 부합하는 보안 연락 정보 엔드포인트를 제공하여, 보안 연구원이 취약점 발견 시 신고 채널을 빠르게 찾을 수 있게 합니다.
...
| Preferred-Languages | 선호 커뮤니케이션 언어 |
| Canonical | 이 파일의 표준 URL |
| Policy | 보안 정책/취약점 공개 정책 링크 |

이 엔드포인트는 레이트 리밋, 인증 등 미들웨어의 제한을 받지 않으며 누구나 직접 접근할 수 있습니다.
...
| SSL/TLS 설정 | 현대적인 TLS 1.2/1.3 설정, 약한 암호화 스위트 비활성화 |
| 백엔드 헤더 숨기기 | `proxy_hide_header`로 webman 버전 등 민감 헤더 제거 |

**사용 방법**: `docs/nginx-security.conf`의 설정을 Nginx server 블록에 병합하고, 실제 도메인과 인증서 경로에 맞게 조정합니다.
...
| 위협 유형 | 공격 벡터 | 방어 계층 |
|----------|---------|---------|
| HTTP 메서드 남용 | TRACE/TRACK XST 공격, CONNECT 터널 프록시, WebDAV 메서드 탐색 | SecurityMiddleware http_method 탐지기 405 메서드 화이트리스트 |
| 표적 무차별 대입 | 특정 사용자를 겨냥한 비밀번호 반복 시도 | 계정 잠금 (5회 실패 시 15분 잠금) + RateLimit (로그인 10/min) + Captcha |
| 무차별 대입 | 분산 IP로 사용자 이름/비밀번호 반복 시도 | RateLimit (로그인 10/min) + Captcha |
...
| SQL 주입 | UNION SELECT, OR 1=1, 주석 우회 | SecurityMiddleware (erikwang2013/security-php) (6가지 패턴) + Eloquent ORM 파라미터화 쿼리 |
| CSRF 크로스 사이트 요청 위조 | 악성 웹사이트의 대리 요청 | SecurityMiddleware (erikwang2013/security-php) Origin/Referer 검증 |
| 경로 탐색 | `../../etc/passwd` | SecurityMiddleware (erikwang2013/security-php) 경로 탐색 패턴 + UploadController 확장자 화이트리스트 |
| 명령 주입 | `;ls`, `` `whoami` ``, `$(cat ...)` | SecurityMiddleware (erikwang2013/security-php) (4가지 패턴) |
| 세션 하이재킹 | JWT Token 탈취 | JWT 단기 유효 (2h) + 블랙리스트 로그아웃 + 민감 작업 비밀번호 이중 확인 |
| ID 열거 | 숫자 ID 순회로 데이터량 추측 | Hashids 난독화로 랜덤 문자열화 |
| 데이터 유출 | DB 탈취 / 중간자 / 로그 유출 | 3계층 암호화/마스킹 + OperationLog 민감 필드 필터링 |
| DoS 공격 | 초대형 요청 본문 / 고빈도 요청 | 요청 본문 10MB 제한 + RateLimit 60/min + IP 블랙리스트 |
| 권한 상승 | 낮은 권한 사용자의 관리 인터페이스 접근 | RBAC method.path 단위 인가 |
...
| 한계 | 영향 범위 | 완화 조치 |
|------|---------|---------|
| CSRF 보호는 브라우저에서만 유효 | 비브라우저 클라이언트 (curl, Postman, 모바일 App)는 Origin/Referer 검사 우회 가능 | 비브라우저 클라이언트는 본래 CSRF 공격에 면역; JWT 인증으로 Cookie 대체 의존 |
| Redis 사용 불가 시 레이트 리밋과 블랙리스트가 fail-open으로 다운그레이드 | 공격자가 레이트 리밋과 고빈도 차단 우회 가능 | Redis 가용성 모니터링 알림; IP 블랙리스트는 file/redis/cache 3종 백엔드로 다운그레이드 지원 |
| 독립 WAF 엔진 없음 | 정규식 기반 탐지로 전용 WAF 규칙 엔진이 아님 | 프로덕션에서는 Nginx ModSecurity 또는 Cloudflare WAF 전면 배치 권장 |
