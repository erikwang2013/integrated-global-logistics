...
- `logistics_admin_user` (table des utilisateurs)
- `logistics_admin_user_role` (table de liaison utilisateurs-rôles)
- `logistics_admin_role` (table des rôles)
- `logistics_admin_role_permission` (table de liaison rôles-permissions)
- `logistics_admin_permission` (table des permissions)
...
## 13. Sécurité du portail client et des paiements (M7–M10)
Vérification de signature des webhooks Stripe / PayPal (HMAC-SHA256 / verify-webhook-signature) ; clés de paiement chiffrées via `Encryptable` dans `logistics_system_config` ; USDT TRC20 vérifié automatiquement via Tronscan, BEP20 / ERC20 manuellement ; X-API-Key défini par le client (≥16 caractères) stocké en sha256. Détection d'attaques au niveau passerelle (M11) : `ecat-security` SecurityBodyLayer intégré à la passerelle, les charges d'attaque sont bloquées au niveau passerelle. **13.5 Sécurité CDN (M12)**: plan gratuit Cloudflare — HTTPS complet + WAF double couche (règles gérées en périphérie + détection applicative passerelle) ; origine Tunnel sans exposition publique ; callbacks via sous-domaine DNS direct pour ne pas perdre de commandes en cas de panne CDN ; limitation de débit par X-API-Key, indépendante des IP de périphérie CDN ; endpoints authentifiés toujours no-store contre le mélange de cache entre utilisateurs. **13.6 Gestion des clés des fournisseurs CDN (M13)**: profils des fournisseurs CDN (access_key / access_secret / liste de domaines) dans la table `logistics_cdn_provider`, champs secrets chiffrés via `Encryptable`, maintenance via CRUD `/admin/cdn/provider`.
