...
| 테이블 이름 | 필드 수 | 설명 |
|------|-------|------|
| `logistics_admin_user` | 14 | 관리 사용자, phone/email/id_card 암호화 저장, 소프트 삭제 지원 |
| `logistics_admin_role` | 7 | 역할, slug 고유 |
| `logistics_admin_permission` | 10 | 권한 트리 (parent_id 자체 참조), type: 1=메뉴 2=버튼 3=API |
| `logistics_admin_user_role` | 2 | 사용자-역할 다대다 중간 테이블 |
| `logistics_admin_role_permission` | 2 | 역할-권한 다대다 중간 테이블 |
| `logistics_system_config` | 8 | 키-값 설정, group+key 복합 고유 |
| `logistics_operation_log` | 9 | 작업 감사 로그 (source 출처 단말 포함) |

### 3.3 기본 키 규칙

- 타입: `BIGINT UNSIGNED NOT NULL`
- 특성: **비자동증가**, Snowflake 알고리즘으로 애플리케이션 계층에서 생성
- 장점: 전역 고유, 분산 환경 친화적, 추세 증가로 인덱스에 유리, 업무량 노출 없음
...
Redis Sorted Set 슬라이딩 윈도우 알고리즘 기반, 원자적 Lua 스크립트 실행:
...
사용자, 역할, 권한 삭제 등 민감 작업은 요청 본문에 현재 사용자 비밀번호를 전달하여 본인 확인을 수행해야 합니다:
...
프론트엔드는 삭제 작업 트리거 전에 확인 다이얼로그를 띄우고 사용자 비밀번호를 수집한 후 요청을 보냅니다.
...
| 페이지 | 라우트 | 설명 |
|------|------|------|
| LoginPage | `pages/LoginPage` | 사용자 이름·비밀번호 + 클릭 캡차 로그인 |
| DashboardPage | `pages/DashboardPage` | 통계 카드 + 최근 작업 |
| UserListPage | `pages/UserListPage` | 사용자 목록, 검색 + 당겨서 새로고침 + 위로 밀어 로드 |
| UserDetailPage | `pages/UserDetailPage` | 추가/편집/조회/삭제 (AlertDialog 확인) |
...
| CSP | Content-Security-Policy 헤더로 리소스 출처 제한, XSS 및 데이터 주입 방지 |
| 작업 확인 | 삭제 등 민감 작업은 현재 사용자 비밀번호 이중 확인 필요 |
| 전송 | HTTPS + JWT Bearer Token |
| 인터페이스 ID | Hashids 암호화, 외부에서 실제 ID 역추론 불가 |
| 요청 본문 | AES-256-CBC 민감 필드 암호화 |
| 데이터베이스 | BIGINT 기본 키 (자동 증가량 노출 안 함) |
...
```
JWT_SECRET          → 환경 변수 주입, 64자리 랜덤 문자열
HASHIDS_SALT        → 유일한 솔트 값, 유출 시 전면 교체 필요
ENCRYPTION_KEY      → API 전송 암호화 키, 32바이트
ENCRYPTABLE_KEY     → DB 저장 암호화 키, 전송 키와 분리
...
| 저장 | phone/email/id_card | encryptable 암호화로 암호문 저장 |
...
| `mysql` | mysql:8.0 | 3306 | 주 데이터베이스, 데이터 볼륨 영속화 |
| `redis` | redis:7-alpine | 6379 | 캐시 / 레이트 리밋 / 캡차 |
...
GitHub Actions 지속적 통합은 `.github/workflows/ci.yml`에 정의됩니다:
- PHP 문법 검사 (`php -l`)
- PHPUnit 단위 테스트
...
`database/backup/backup.sh` — mysqldump + gzip 백업, 30일 전 이전 백업 자동 정리.
`database/backup/restore.sh` — 대화형 선택 후 백업 복구.

### 8.5 모니터링

`GET /metrics` 엔드포인트 (`MetricsController`)가 Prometheus text format으로 5개 gauge 지표를 노출합니다: HTTP 요청 총수, 활성 사용자 수, DB/Redis 연결 상태, 메모리 사용량.
