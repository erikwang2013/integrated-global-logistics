...
flowchart TB
    ...
    subgraph "طبقة التخزين"
        D1[("MySQL 8.0<br/>التخزين الرئيسي<br/>بادئة الجداول logistics_")]
        D2[("Elasticsearch<br/>البحث النصي الكامل<br/>بادئة الفهارس logistics_")]
...
flowchart TD
    ...
    subgraph "طبقة معالجة الاستثناءات (Exception Layer)"
        E1["app/exception/Handler<br/>طلبات المتصفح → صفحة خطأ بحيوان المشروع<br/>طلبات API / وضع التصحيح → السلوك الافتراضي لإطار العمل"]
    end
...
sequenceDiagram
    ...
    C->>N: طلب HTTPS<br/>Header: Accept-Language: zh_CN
...
flowchart LR
    ...
    subgraph "2. التخزين"
        S1["جداول MySQL logistics_*<br/>id BIGINT UNSIGNED<br/>NOT NULL"]
...
flowchart TB
    ...
    subgraph "طبقة البيانات"
        MYSQL["MySQL 8.0<br/>نسخ رئيسي-تابع<br/>بادئة logistics_"]
        ES["Elasticsearch 8.x<br/>مجموعة من 3 عقد<br/>بادئة logistics_"]
