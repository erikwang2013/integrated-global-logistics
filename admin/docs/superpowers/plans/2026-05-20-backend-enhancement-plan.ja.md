...
**Goal:** 13 項目のバックエンド拡張を実装：3 個の新規ミドルウェア (CORS, RateLimit, OperationLog)、6 個の新規コントローラー、2 個のモデル修正、ルート + ミドルウェア設定。

**Architecture:** 既存の webman パターンに従う — ミドルウェアは `MiddlewareInterface::process()` を実装、コントローラーは `BaseController` を継承、ルートはクロージャまたは `[class, method]` で定義。グローバルミドルウェアは `config/middleware.php` に登録し、ルートレベルのミドルウェアは `config/route.php` のルートグループにマウントする。
...
`app/model/AdminUser.php` の 10 行目の後に 2 つの `use` インポートを挿入し、クラス本体の先頭で 2 つの trait を使用します：
...
同時にクラスの末尾（`roles()` メソッドの後、クラスの閉じ括弧の前）に `toSearchableArray()` メソッドを追加します：
...
`config/middleware.php` の内容を次のように変更：
...
`AdminAuth::process()` メソッド内で、token 抽出の後、JWT デコードの前にブラックリストチェックを挿入します：
...
Redis のインポートも同時に追加する必要があります。ファイルヘッダーを修正：
...
### Task 13: ルート更新 — すべての新規ルート + ミドルウェアバインド
...
`/admin` ルートグループ内に新規ルートを追加し、OperationLog ミドルウェアを同時に追加します。完全なルートファイル：
