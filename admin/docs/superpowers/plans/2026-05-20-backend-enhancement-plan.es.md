...
# Mejoras del backend — Plan de implementación

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Implementar 13 mejoras del backend: 3 nuevos middleware (CORS, RateLimit, OperationLog), 6 nuevos controladores, 2 modificaciones de modelos, rutas + configuración de middleware.

**Architecture:** Seguir el patrón webman existente: el middleware implementa `MiddlewareInterface::process()`, los controladores heredan de `BaseController`, y las rutas se definen con closures o `[class, method]`. El middleware global se registra en `config/middleware.php`, y el middleware de nivel de ruta se monta en los grupos de rutas de `config/route.php`.

**Tech Stack:** PHP 8.3+, webman v2, Redis (extensión), PhpSpreadsheet (importación)

**Orden de dependencias:** modificación de modelos → middleware → controladores → rutas/configuración
...
Insertar dos imports `use` después de la línea 10 de `app/model/AdminUser.php` y usar los dos traits al principio del cuerpo de la clase:
...
Al mismo tiempo, añadir el método `toSearchableArray()` al final de la clase (después del método `roles()`, antes de la llave de cierre de la clase):
...
### Tarea 3: Middleware de límite de peticiones RateLimit
...
### Tarea 4: Middleware de logs de operaciones OperationLog
...
### Tarea 5: Configuración del middleware global

**Archivos:**
- Modificar: `config/middleware.php`

- [ ] **Paso 1: Registrar el middleware global**
...
### Tarea 6: Controlador de comprobación de salud
...
### Tarea 8: Controlador de consulta de logs de operaciones
...
### Tarea 9: Controlador del centro personal (incluye logout)

**Archivos:**
- Crear: `app/admin/controller/ProfileController.php`
- Modificar: `app/middleware/AdminAuth.php` (validación de lista negra JWT)

- [ ] **Paso 1: Modificar el middleware AdminAuth para añadir la validación de lista negra JWT**
...
También hay que añadir el import de Redis. Modificar el encabezado del archivo:
...
### Tarea 10: Controlador de subida de archivos
...
### Tarea 11: Operaciones por lotes de usuarios
...
### Tarea 12: Controlador de importación de Excel
...
### Tarea 13: Actualización de rutas — todas las rutas nuevas + enlace de middleware
...
Añadir las rutas nuevas dentro del grupo de rutas `/admin` y añadir el middleware OperationLog. Archivo de rutas completo:
