...
**Goal:** Implement 13 backend enhancements: 3 new middleware (CORS, RateLimit, OperationLog), 6 new controllers, 2 model changes, and route + middleware configuration.

**Architecture:** Follow the existing webman patterns — middleware implements `MiddlewareInterface::process()`, controllers extend `BaseController`, and routes are defined with closures or `[class, method]`. Global middleware is registered in `config/middleware.php`, and route-level middleware is mounted on route groups in `config/route.php`.

**Tech Stack:** PHP 8.3+, webman v2, Redis (extension), PhpSpreadsheet (import)

**Dependency order:** model changes → middleware → controllers → routes/config

---

### Task 1: Add SoftDeletes + Searchable trait to the AdminUser model
...
Also add a `toSearchableArray()` method at the end of the class (after the `roles()` method, before the class closing brace):
...
### Task 3: RateLimit Rate-limiting Middleware
...
### Task 4: OperationLog Operation Log Middleware
...
### Task 9: Profile Controller (incl. Logout)

**Files:**
- Create: `app/admin/controller/ProfileController.php`
- Modify: `app/middleware/AdminAuth.php` (JWT blacklist check)

- [ ] **Step 1: Modify the AdminAuth middleware to add JWT blacklist checking**

Insert the blacklist check in the `AdminAuth::process()` method, after token extraction and before JWT decoding:
...
The Redis import also needs to be added. Modify the file header:
