...
# تحسين الواجهة الخلفية — خطة التنفيذ

> **للعاملين الوكيلين (agentic workers):** المهارة الفرعية المطلوبة: استخدم superpowers:subagent-driven-development (موصى بها) أو superpowers:executing-plans لتنفيذ هذه الخطة مهمة بمهمة. تستخدم الخطوات صيغة مربعات الاختيار (`- [ ]`) للتتبع.

**الهدف:** تنفيذ 13 تحسينًا للواجهة الخلفية: 3 وسائط جديدة (CORS, RateLimit, OperationLog)، و6 وحدات تحكم جديدة، وتعديلان على النماذج، وتكوين التوجيه + الوسائط.

**البنية:** اتباع نمط webman الحالي — تنفذ الوسائط `MiddlewareInterface::process()`، وترث وحدات التحكم من `BaseController`، وتُعرَّف المسارات عبر دوال الإغلاق أو `[class, method]`. تُسجل الوسائط العامة في `config/middleware.php`، بينما تُركب وسائط مستوى المسار على مجموعات المسارات في `config/route.php`.

**حزمة التقنيات:** PHP 8.3+, webman v2, Redis (امتداد), PhpSpreadsheet (استيراد)

**ترتيب الاعتماديات:** تعديل النماذج ← الوسائط ← وحدات التحكم ← التوجيه/الإعدادات
...
إدراج استيرادين `use` بعد السطر 10 في `app/model/AdminUser.php`، واستخدام الترايتين في مقدمة جسم الفئة:
...
وفي نهاية الفئة (بعد دالة `roles()` وقبل قوس إغلاق الفئة) إضافة دالة `toSearchableArray()`:
...
### Task 3: وسيط الحد من المعدل RateLimit
...
### Task 4: وسيط سجل العمليات OperationLog
...
### Task 7: وحدة تحكم CRUD لإعدادات النظام
...
### Task 9: وحدة تحكم الحساب الشخصي (بما في ذلك تسجيل الخروج)

**الملفات:**
- إنشاء: `app/admin/controller/ProfileController.php`
- تعديل: `app/middleware/AdminAuth.php` (التحقق من القائمة السوداء لـ JWT)

- [ ] **الخطوة 1: تعديل وسيط AdminAuth وإضافة التحقق من القائمة السوداء لـ JWT**

في دالة `AdminAuth::process()`، إدراج فحص القائمة السوداء بعد استخراج token وقبل فك ترميز JWT:
...
يتطلب الأمر أيضًا إضافة استيراد Redis. تعديل رأس الملف:
...
- [ ] **الخطوة 1: إضافة دالتين في نهاية فئة UserController (بعد دالة `destroy`)**
...
### Task 13: تحديث التوجيه — جميع المسارات الجديدة + ربط الوسائط

**الملفات:**
- تعديل: `config/route.php`

- [ ] **الخطوة 1: تحديث إعداد التوجيه**

إضافة مسارات جديدة داخل مجموعة `/admin` مع إلحاق وسيط OperationLog. ملف التوجيه الكامل:
...
بعد اكتمال التنفيذ، تحقق من كل بند:
