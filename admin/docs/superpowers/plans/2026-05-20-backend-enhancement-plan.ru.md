...
# Доработка бэкенда — план реализации

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Реализовать 13 пунктов доработки бэкенда: 3 новых промежуточных ПО (CORS, RateLimit, OperationLog), 6 новых контроллеров, 2 изменения моделей, конфигурация маршрутов + промежуточного ПО.

**Architecture:** Следовать существующим паттернам webman — промежуточное ПО реализует `MiddlewareInterface::process()`, контроллеры наследуют `BaseController`, маршруты определяются через замыкания или `[class, method]`. Глобальное промежуточное ПО регистрируется в `config/middleware.php`, промежуточное ПО уровня маршрутов подключается на группы маршрутов в `config/route.php`.

**Tech Stack:** PHP 8.3+, webman v2, Redis (расширение), PhpSpreadsheet (импорт)

**Порядок зависимостей:** изменения моделей → промежуточное ПО → контроллеры → маршруты/конфигурация

---

### Task 1: Добавление в модель AdminUser SoftDeletes + Searchable trait

**Files:**
- Modify: `app/model/AdminUser.php`

- [ ] **Step 1: изменение модели AdminUser**

После 10-й строки файла `app/model/AdminUser.php` вставить два импорта `use`, и в начале тела класса задействовать два trait:
...
Также в конец класса (после метода `roles()`, перед закрывающей фигурной скобкой) добавить метод `toSearchableArray()`:
...
### Task 2: CORS-промежуточное ПО

**Files:**
- Create: `app/middleware/Cors.php`

- [ ] **Step 1: создание Cors-промежуточного ПО**
...
### Task 3: RateLimit-промежуточное ПО ограничения частоты

**Files:**
- Create: `app/middleware/RateLimit.php`

- [ ] **Step 1: создание RateLimit-промежуточного ПО**
...
### Task 4: OperationLog-промежуточное ПО журнала операций

**Files:**
- Create: `app/middleware/OperationLog.php`

- [ ] **Step 1: создание OperationLog-промежуточного ПО**
...
### Task 5: Конфигурация глобального промежуточного ПО

**Files:**
- Modify: `config/middleware.php`

- [ ] **Step 1: регистрация глобального промежуточного ПО**
...
### Task 6: Контроллер проверки работоспособности

**Files:**
- Create: `app/admin/controller/HealthController.php`

- [ ] **Step 1: создание HealthController**
...
### Task 7: Контроллер CRUD системной конфигурации

**Files:**
- Create: `app/admin/controller/ConfigController.php`

- [ ] **Step 1: создание ConfigController**
...
### Task 8: Контроллер просмотра журнала операций

**Files:**
- Create: `app/admin/controller/LogController.php`

- [ ] **Step 1: создание LogController**
...
### Task 9: Контроллер личного кабинета (включая выход)

**Files:**
- Create: `app/admin/controller/ProfileController.php`
- Modify: `app/middleware/AdminAuth.php` (проверка JWT-черного списка)

- [ ] **Step 1: изменение AdminAuth-промежуточного ПО, добавление проверки JWT-черного списка**

В метод `AdminAuth::process()`, после извлечения token и до декодирования JWT, вставить проверку черного списка:

```php
// вставить между строками 30-31 (после извлечения token, до декодирования JWT):
        // проверить JWT-черный список
...
Также нужно добавить импорт Redis. Изменить начало файла:
...
- [ ] **Step 2: создание ProfileController**
...
### Task 10: Контроллер загрузки файлов

**Files:**
- Create: `app/admin/controller/UploadController.php`

- [ ] **Step 1: создание UploadController**
...
### Task 11: Пакетные операции с пользователями

**Files:**
- Modify: `app/admin/controller/UserController.php`

- [ ] **Step 1: добавить в конец класса UserController два метода (после метода `destroy`)**
...
### Task 12: Контроллер импорта Excel

**Files:**
- Create: `app/admin/controller/ImportController.php`

- [ ] **Step 1: создание ImportController**
...
### Task 13: Обновление маршрутов — все новые маршруты + привязка промежуточного ПО

**Files:**
- Modify: `config/route.php`

- [ ] **Step 1: обновление конфигурации маршрутов**

Добавить новые маршруты в группу `/admin` и подключить OperationLog-промежуточное ПО. Полный файл маршрутов:
...
После завершения реализации проверять по пунктам:
