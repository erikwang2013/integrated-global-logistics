...
# ব্যাকএন্ড এনহ্যান্সমেন্ট — ইমপ্লিমেন্টেশন প্ল্যান

> **For agentic workers:** বাধ্যতামূলক সাব-স্কিল: এই প্ল্যানটি ধাপে ধাপে বাস্তবায়নের জন্য superpowers:subagent-driven-development (প্রস্তাবিত) অথবা superpowers:executing-plans ব্যবহার করুন। ধাপগুলো ট্র্যাকিংয়ের জন্য চেকবক্স (`- [ ]`) সিনট্যাক্স ব্যবহার করা হয়।

**Goal (লক্ষ্য):** 13টি ব্যাকএন্ড এনহ্যান্সমেন্ট বাস্তবায়ন: 3টি নতুন মিডলওয়্যার (CORS, RateLimit, OperationLog), 6টি নতুন কন্ট্রোলার, 2টি মডেল পরিবর্তন, রাউট + মিডলওয়্যার কনফিগারেশন।

**Architecture (স্থাপত্য):** বিদ্যমান webman প্যাটার্ন অনুসরণ করুন — মিডলওয়্যার `MiddlewareInterface::process()` বাস্তবায়ন করে, কন্ট্রোলার `BaseController` উত্তরাধিকারসূত্রে গ্রহণ করে, রাউট ক্লোজার বা `[class, method]` দিয়ে সংজ্ঞায়িত হয়। গ্লোবাল মিডলওয়্যার `config/middleware.php`-এ রেজিস্টার করা হয়, রাউট-লেভেল মিডলওয়্যার `config/route.php`-এর রাউট গ্রুপে মাউন্ট হয়।

**Tech Stack (প্রযুক্তি স্ট্যাক):** PHP 8.3+, webman v2, Redis (এক্সটেনশন), PhpSpreadsheet (ইমপোর্ট)

**ডিপেন্ডেন্সি ক্রম:** মডেল পরিবর্তন → মিডলওয়্যার → কন্ট্রোলার → রাউট/কনফিগ

---

### Task 1: AdminUser মডেলে SoftDeletes + Searchable trait যোগ করুন

**Files:**
- Modify: `app/model/AdminUser.php`

- [ ] **Step 1: AdminUser মডেল পরিবর্তন করুন**

`app/model/AdminUser.php`-এর 10 নম্বর লাইনের পরে দুটি `use` ইমপোর্ট ঢোকান এবং ক্লাস বডির একদম শুরুতে দুটি trait ব্যবহার করুন:
...
একই সাথে ক্লাসের শেষে (`roles()` মেথডের পরে, ক্লাসের সমাপ্তি কার্লি ব্রেসের আগে) `toSearchableArray()` মেথড যোগ করুন:
...
- [ ] **Step 1: Cors মিডলওয়্যার তৈরি করুন**
...
- [ ] **Step 1: RateLimit মিডলওয়্যার তৈরি করুন**
...
- [ ] **Step 1: OperationLog মিডলওয়্যার তৈরি করুন**
...
- [ ] **Step 1: গ্লোবাল মিডলওয়্যার রেজিস্টার করুন**

`config/middleware.php`-এর কনটেন্ট নিম্নরূপ পরিবর্তন করুন:
...
- [ ] **Step 1: HealthController তৈরি করুন**
...
- [ ] **Step 1: ConfigController তৈরি করুন**
...
- [ ] **Step 1: LogController তৈরি করুন**
...
### Task 9: ব্যক্তিগত প্রোফাইল কন্ট্রোলার (লগআউটসহ)

**Files:**
- Create: `app/admin/controller/ProfileController.php`
- Modify: `app/middleware/AdminAuth.php` (JWT ব্ল্যাকলিস্ট যাচাই)

- [ ] **Step 1: AdminAuth মিডলওয়্যার পরিবর্তন করুন, JWT ব্ল্যাকলিস্ট যাচাই যোগ করুন**

`AdminAuth::process()` মেথডে, token এক্সট্রাকশনের পরে, JWT ডিকোডের আগে ব্ল্যাকলিস্ট চেক ঢোকান:
...
একই সাথে Redis ইমপোর্ট যোগ করতে হবে। ফাইল হেডার পরিবর্তন করুন:
...
- [ ] **Step 2: ProfileController তৈরি করুন**
...
- [ ] **Step 1: UploadController তৈরি করুন**
...
- [ ] **Step 1: ImportController তৈরি করুন**
...
### Task 13: রাউট আপডেট — সব নতুন রাউট + মিডলওয়্যার বাইন্ডিং

**Files:**
- Modify: `config/route.php`

- [ ] **Step 1: রাউট কনফিগারেশন আপডেট করুন**

`/admin` রাউট গ্রুপে নতুন রাউট যোগ করুন, একই সাথে OperationLog মিডলওয়্যার যুক্ত করুন। সম্পূর্ণ রাউট ফাইল:
...
ইমপ্লিমেন্টেশন শেষ হলে প্রতিটি আইটেম যাচাই করুন:
