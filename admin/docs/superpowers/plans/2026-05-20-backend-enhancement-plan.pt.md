...
**Goal:** implementar 13 aprimoramentos do backend: 3 novos middlewares (CORS, RateLimit, OperationLog), 6 novos controladores, 2 modificações de modelos, configuração de rotas + middleware.

**Architecture:** seguir o padrão webman existente — middlewares implementam `MiddlewareInterface::process()`, controladores herdam de `BaseController`, rotas definidas com closures ou `[class, method]`. Os middlewares globais são registrados em `config/middleware.php`, os middlewares de rota são montados nos grupos de rotas em `config/route.php`.
...
### Task 1: Adicionar SoftDeletes + trait Searchable ao modelo AdminUser
...
Inserir duas importações `use` após a linha 10 de `app/model/AdminUser.php` e usar os dois traits no início do corpo da classe:
...
Ao mesmo tempo, adicionar o método `toSearchableArray()` no final da classe (após o método `roles()`, antes da chave de fechamento da classe):
...
### Task 4: Middleware de log de operação OperationLog
...
### Task 5: Configuração dos middlewares globais
...
### Task 6: Controlador de verificação de saúde
...
### Task 7: Controlador CRUD de configuração do sistema
...
### Task 8: Controlador de consulta de logs de operação
...
- Modify: `app/middleware/AdminAuth.php` (verificação de blacklist JWT)

- [ ] **Step 1: Modificar o middleware AdminAuth, adicionar verificação de blacklist JWT**

No método `AdminAuth::process()`, inserir a verificação da blacklist após a extração do token e antes da decodificação JWT:
...
Também é necessário adicionar a importação do Redis. Alterar o cabeçalho do arquivo:
...
### Task 13: Atualização de rotas — todas as novas rotas + vínculo de middleware
...
Adicionar novas rotas dentro do grupo de rotas `/admin`, além de acrescentar o middleware OperationLog. Arquivo de rotas completo:
