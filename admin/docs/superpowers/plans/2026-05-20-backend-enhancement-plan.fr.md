...
# Améliorations du backend — Plan d'implémentation

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Implémenter 13 améliorations du backend : 3 nouveaux middlewares (CORS, RateLimit, OperationLog), 6 nouveaux contrôleurs, 2 modifications de modèles, configuration des routes + middlewares.

**Architecture:** Suivre les conventions webman existantes — les middlewares implémentent `MiddlewareInterface::process()`, les contrôleurs héritent de `BaseController`, les routes se définissent par fermeture ou `[class, method]`. Les middlewares globaux sont enregistrés dans `config/middleware.php`, les middlewares de niveau route sont montés sur les groupes de routes dans `config/route.php`.

**Tech Stack:** PHP 8.3+, webman v2, Redis (extension), PhpSpreadsheet (import)

**Ordre des dépendances :** modèles → middlewares → contrôleurs → routes/configuration

---

### Tâche 1 : ajouter les traits SoftDeletes + Searchable au modèle AdminUser
...
Insérer deux importations `use` après la ligne 10 de `app/model/AdminUser.php`, puis utiliser les deux traits en tête du corps de la classe :
...
Ajouter également la méthode `toSearchableArray()` à la fin de la classe (après la méthode `roles()`, avant l'accolade fermante) :
...
### Tâche 3 : middleware de limitation de débit RateLimit
...
### Tâche 4 : middleware de journalisation des opérations OperationLog
...
### Tâche 6 : contrôleur de vérification de santé
...
### Tâche 9 : contrôleur de l'espace personnel (avec déconnexion)
...
Dans la méthode `AdminAuth::process()`, insérer le contrôle de la liste noire après l'extraction du jeton et avant le décodage JWT :
...
Il faut également ajouter l'importation de Redis. Modifier l'en-tête du fichier :
...
### Tâche 10 : contrôleur de téléversement de fichiers
...
### Tâche 13 : mise à jour des routes — toutes les nouvelles routes + liaison des middlewares
...
Ajouter les nouvelles routes dans le groupe `/admin` et y attacher le middleware OperationLog. Fichier de routes complet :
...
Une fois l'implémentation terminée, vérifier point par point :
