...
> **Untuk pekerja agen:** SUB-KETERAMPILAN WAJIB: Gunakan superpowers:subagent-driven-development (disarankan) atau superpowers:executing-plans untuk mengimplementasikan rencana ini tugas demi tugas. Langkah-langkah menggunakan sintaks kotak centang (`- [ ]`) untuk pelacakan.

**Tujuan:** Mengimplementasikan 13 peningkatan backend: 3 middleware baru (CORS, RateLimit, OperationLog), 6 controller baru, 2 modifikasi model, konfigurasi rute + middleware.
...
### Tugas 1: Menambahkan trait SoftDeletes + Searchable pada model AdminUser

**File:**
- Modify: `app/model/AdminUser.php`

- [ ] **Langkah 1: Memodifikasi model AdminUser**

Masukkan dua impor `use` setelah baris ke-10 di `app/model/AdminUser.php`, dan gunakan dua trait di bagian paling awal tubuh kelas:
...
Selain itu, tambahkan metode `toSearchableArray()` di akhir kelas (setelah metode `roles()`, sebelum kurung kurawal penutup kelas):
...
- [ ] **Langkah 2: Memverifikasi sintaks**

```bash
php -l app/model/AdminUser.php
```
Diharapkan: `No syntax errors detected`
...
### Tugas 2: Middleware CORS

**File:**
- Create: `app/middleware/Cors.php`

- [ ] **Langkah 1: Membuat middleware Cors**
...
### Tugas 3: Middleware Pembatasan Laju RateLimit

**File:**
- Create: `app/middleware/RateLimit.php`

- [ ] **Langkah 1: Membuat middleware RateLimit**
...
### Tugas 4: Middleware Log Operasi OperationLog

**File:**
- Create: `app/middleware/OperationLog.php`

- [ ] **Langkah 1: Membuat middleware OperationLog**
...
### Tugas 5: Konfigurasi Middleware Global

**File:**
- Modify: `config/middleware.php`

- [ ] **Langkah 1: Mendaftarkan middleware global**
...
### Tugas 6: Controller Pemeriksaan Kesehatan

**File:**
- Create: `app/admin/controller/HealthController.php`

- [ ] **Langkah 1: Membuat HealthController**
...
### Tugas 7: Controller CRUD Konfigurasi Sistem

**File:**
- Create: `app/admin/controller/ConfigController.php`

- [ ] **Langkah 1: Membuat ConfigController**
...
### Tugas 8: Controller Kueri Log Operasi

**File:**
- Create: `app/admin/controller/LogController.php`

- [ ] **Langkah 1: Membuat LogController**
...
### Tugas 9: Controller Pusat Profil (termasuk logout)

**File:**
- Create: `app/admin/controller/ProfileController.php`
- Modify: `app/middleware/AdminAuth.php` (validasi daftar hitam JWT)

- [ ] **Langkah 1: Memodifikasi middleware AdminAuth, menambahkan validasi daftar hitam JWT**

Dalam metode `AdminAuth::process()`, masukkan pemeriksaan daftar hitam setelah ekstraksi token dan sebelum dekode JWT:
...
Perlu menambahkan impor Redis sekaligus. Ubah header file:
...
- [ ] **Langkah 2: Membuat ProfileController**
...
- [ ] **Langkah 3: Memverifikasi sintaks**
...
### Tugas 10: Controller Unggah File

**File:**
- Create: `app/admin/controller/UploadController.php`

- [ ] **Langkah 1: Membuat UploadController**
...
### Tugas 11: Operasi Batch Pengguna

**File:**
- Modify: `app/admin/controller/UserController.php`

- [ ] **Langkah 1: Menambahkan dua metode di akhir kelas UserController (setelah metode `destroy`)**
...
### Tugas 12: Controller Impor Excel

**File:**
- Create: `app/admin/controller/ImportController.php`

- [ ] **Langkah 1: Membuat ImportController**
...
### Tugas 13: Pembaruan Rute — Semua rute baru + pengikatan middleware

**File:**
- Modify: `config/route.php`

- [ ] **Langkah 1: Memperbarui konfigurasi rute**
