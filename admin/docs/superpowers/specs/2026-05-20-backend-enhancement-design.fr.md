...
La table `logistics_operation_log` ne possède que la colonne `created_at` (pas de `updated_at`). Par défaut, Eloquent tente d'écrire `updated_at` dans `save()`, ce qui provoque une erreur SQL.
