...
flowchart TD
    ...
    subgraph "例外処理層 (Exception Layer)"
        E1["app/exception/Handler<br/>ブラウザリクエスト → プロジェクトペットのエラーページ<br/>API リクエスト / デバッグモード → フレームワーク既定動作"]
    end
...
sequenceDiagram
    ...
    C->>N: HTTPS リクエスト<br/>Header: Accept-Language: zh_CN
    N->>MW_LOC: 転送
