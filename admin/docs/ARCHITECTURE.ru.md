...
flowchart TB
    ...
    subgraph "Уровень хранения"
        D1[("MySQL 8.0<br/>Основное хранилище<br/>Префикс таблиц logistics_")]
        D2[("Elasticsearch<br/>Полнотекстовый поиск<br/>Префикс индексов logistics_")]
...
flowchart TD
    ...
    subgraph "Обработка исключений (Exception Layer)"
        E1["app/exception/Handler<br/>запросы браузера → страница ошибки с талисманом<br/>API-запросы / режим отладки → поведение фреймворка по умолчанию"]
    end
...
flowchart LR
    ...
    subgraph "2. Хранение"
        S1["Таблицы MySQL logistics_*<br/>id BIGINT UNSIGNED<br/>NOT NULL"]
...
flowchart TB
    ...
    subgraph "Уровень данных"
        MYSQL["MySQL 8.0<br/>мастер-реплика<br/>префикс logistics_"]
        ES["Elasticsearch 8.x<br/>кластер из 3 узлов<br/>префикс logistics_"]
