...
class Grpc extends Http2
{
     ...
     * @var bool
     */
    private static bool $hookLoaded = false;
    ...
    public static function onRequest(Request $request)
    {
        if (in_array($request->path(), self::$streaming["simple"])) { //普通模式
            // 空消息（如 Carriers 无字段请求）也要走 handler，不能因 unpack 返回空串而漏掉
            $data = self::unpack($request->rawBody());
            if (is_callable(self::$route[$request->path()])) {
                $obj = new  self::$parameter[$request->path()];
                // 生成的 protobuf 消息类无 metadata 属性（magic __set 抛异常），头信息改为第二参数传入
                $obj->mergeFromString($data);
                $responseMessage = (self::$route[$request->path()])($obj, $request->header());
                $data = $responseMessage->serializeToString();
     ...
     * @throws \Exception
     */
    protected static function loadHook()
    {
        // idempotent：property_exists 触发 autoload 会 include 生成类文件，重复执行导致类重声明
        if (self::$hookLoaded) {
            return;
        }
        self::$hookLoaded = true;
